...
function adminScript() {
  return "let DATA = { site: {}, shops: [], notices: [], images: [] };\nlet FILES = [];\nlet currentFilePath = '';\n\nfunction $(id) { return document.getElementById(id); }\nfunction esc(s) {\n  return String(s || '').replace(/[&<>\"']/g, function(m) {\n    return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',\"'\":'&#39;'}[m];\n  });\n}\nfunction toast(msg) {\n  var el = $('toast');\n  if (!el) return alert(msg);\n  el.textContent = msg;\n  el.classList.remove('hide');\n  setTimeout(function(){ el.classList.add('hide'); }, 2200);\n}\nasync function api(url, opt) {\n  opt = opt || {};\n  const r = await fetch(url, {\n    headers: { 'Content-Type': 'application/json', ...(opt.headers || {}) },\n    ...opt\n  });\n  const j = await r.json().catch(function(){ return { ok:false, error:'응답 오류' }; });\n  if (!r.ok || j.ok === false) throw new Error(j.error || '처리 실패');\n  return j;\n}\n\nfunction showTab(t) {\n  document.querySelectorAll('.nav button').forEach(function(b){\n    b.classList.toggle('active', b.getAttribute('data-tab') === t);\n  });\n  document.querySelectorAll('.tab').forEach(function(s){ s.classList.add('hide'); });\n  var tab = $('tab-' + t);\n  if (tab) tab.classList.remove('hide');\n  var btn = document.querySelector('.nav button[data-tab=\"' + t + '\"]');\n  if (btn && $('pageTitle')) $('pageTitle').textContent = btn.textContent;\n  if (t === 'files') loadFiles();\n}\n\ndocument.addEventListener('DOMContentLoaded', function(){\n  document.querySelectorAll('.nav button').forEach(function(b){\n    b.addEventListener('click', function(){ showTab(b.getAttribute('data-tab')); });\n  });\n  var shopImage = $('shopImage');\n  if (shopImage) shopImage.addEventListener('input', previewShopImage);\n  loadAll();\n});\n\nasync function loadAll() {\n  try {\n    const j = await api('/admin/api/data');\n    DATA = j.data || { site:{}, shops:[], notices:[], images:[] };\n    fillSite();\n    renderStats();\n    renderShops();\n    renderNotices();\n    renderImages();\n  } catch(e) { toast(e.message); }\n}\nfunction renderStats() {\n  if ($('statShops')) $('statShops').textContent = (DATA.shops || []).length + '개';\n  if ($('statNotices')) $('statNotices').textContent = (DATA.notices || []).length + '개';\n  if ($('statImages')) $('statImages').textContent = (DATA.images || []).length + '개';\n}\nfunction fillSite() {\n  var s = DATA.site || {};\n  if ($('siteTitle')) $('siteTitle').value = s.title || '';\n  if ($('siteDesc')) $('siteDesc').value = s.description || '';\n  if ($('sitePhone')) $('sitePhone').value = s.phone || '';\n  if ($('siteKakao')) $('siteKakao').value = s.kakao || '';\n  if ($('siteFooter')) $('siteFooter').value = s.footerText || '';\n  if ($('siteInject')) $('siteInject').value = String(s.injectManagedSection !== false);\n}\nasync function saveSite() {\n  try {\n    const site = {\n      title: $('siteTitle').value,\n      description: $('siteDesc').value,\n      phone: $('sitePhone').value,\n      kakao: $('siteKakao').value,\n      footerText: $('siteFooter').value,\n      injectManagedSection: $('siteInject').value === 'true'\n    };\n    await api('/admin/api/site', { method:'POST', body: JSON.stringify(site) });\n    toast('사이트 설정 저장 완료');\n    loadAll();\n  } catch(e) { toast(e.message); }\n}\n\nfunction resetShopForm() {\n  ['shopId','shopName','shopArea','shopCategory','shopPhone','shopImage','shopSlug','shopDesc','shopBody'].forEach(function(id){ if ($(id)) $(id).value = ''; });\n  if ($('shopOrder')) $('shopOrder').value = '10';\n  if ($('shopActive')) $('shopActive').value = 'true';\n  if ($('shopPreview')) $('shopPreview').classList.add('hide');\n}\nfunction editShop(id) {\n  const s = (DATA.shops || []).find(function(x){ return x.id === id; });\n  if (!s) return;\n  $('shopId').value = s.id;\n  $('shopName').value = s.name || '';\n  $('shopArea').value = s.area || '';\n  $('shopCategory').value = s.category || '';\n  $('shopPhone').value = s.phone || '';\n  $('shopOrder').value = s.order || 10;\n  $('shopActive').value = String(s.active !== false);\n  $('shopImage').value = s.image || '';\n  $('shopSlug').value = s.slug || '';\n  $('shopDesc').value = s.description || '';\n  $('shopBody').value = s.body || '';\n  previewShopImage();\n  window.scrollTo(0,0);\n}\nfunction previewShopImage() {\n  var prev = $('shopPreview');\n  if (!prev || !$('shopImage')) return;\n  var v = $('shopImage').value;\n  if (v) { prev.src = v; prev.classList.remove('hide'); }\n  else prev.classList.add('hide');\n}\nasync function saveShop() {\n  try {\n    const shop = {\n      id: $('shopId').value,\n      name: $('shopName').value,\n      area: $('shopArea').value,\n      category: $('shopCategory').value,\n      phone: $('shopPhone').value,\n      order: Number($('shopOrder').value || 10),\n      active: $('shopActive').value === 'true',\n      image: $('shopImage').value,\n      slug: $('shopSlug').value,\n      description: $('shopDesc').value,\n      body: $('shopBody').value\n    };\n    await api('/admin/api/shop', { method:'POST', body: JSON.stringify(shop) });\n    toast('업소 저장 완료');\n    resetShopForm();\n    loadAll();\n  } catch(e) { toast(e.message); }\n}\nasync function deleteShop(id) {\n  if (!confirm('이 업소를 삭제할까요?')) return;\n  try {\n    await api('/admin/api/shop-delete', { method:'POST', body: JSON.stringify({ id:id }) });\n    toast('삭제 완료');\n    loadAll();\n  } catch(e) { toast(e.message); }\n}\nfunction renderShops() {\n  var rows = (DATA.shops || []).map(function(s){\n    var image = s.image ? '<img src=\"' + esc(s.image) + '\">' : '';\n    return '<tr><td>' + image + '</td><td><b>' + esc(s.name) + '</b><br><span class=\"muted\">/shop/' + esc(s.slug || s.id) + '</span></td><td>' + esc(s.area) + '<br>' + esc(s.category) + '</td><td>' + (s.active !== false ? '<span class=\"pill\">노출</span>' : '<span class=\"pill\">숨김</span>') + '</td><td><button type=\"button\" class=\"btn light js-edit-shop\" data-id=\"' + esc(s.id) + '\">수정</button> <button type=\"button\" class=\"btn danger js-delete-shop\" data-id=\"' + esc(s.id) + '\">삭제</button></td></tr>';\n  }).join('');\n  var body = $('shopRows');\n  if (!body) return;\n  body.innerHTML = rows || '<tr><td colspan=\"5\" class=\"muted\">등록된 업소가 없습니다.</td></tr>';\n  body.querySelectorAll('.js-edit-shop').forEach(function(btn){ btn.addEventListener('click', function(){ editShop(btn.getAttribute('data-id')); }); });\n  body.querySelectorAll('.js-delete-shop').forEach(function(btn){ btn.addEventListener('click', function(){ deleteShop(btn.getAttribute('data-id')); }); });\n}\n\nfunction resetNoticeForm() {\n  if ($('noticeId')) $('noticeId').value = '';\n  if ($('noticeTitle')) $('noticeTitle').value = '';\n  if ($('noticeBody')) $('noticeBody').value = '';\n  if ($('noticeActive')) $('noticeActive').value = 'true';\n}\nfunction editNotice(id) {\n  const n = (DATA.notices || []).find(function(x){ return x.id === id; });\n  if (!n) return;\n  $('noticeId').value = n.id;\n  $('noticeTitle').value = n.title || '';\n  $('noticeBody').value = n.body || '';\n  $('noticeActive').value = String(n.active !== false);\n  window.scrollTo(0,0);\n}\nasync function saveNotice() {\n  try {\n    const notice = { id:$('noticeId').value, title:$('noticeTitle').value, body:$('noticeBody').value, active:$('noticeActive').value === 'true' };\n    await api('/admin/api/notice', { method:'POST', body: JSON.stringify(notice) });\n    toast('공지 저장 완료');\n    resetNoticeForm();\n    loadAll();\n  } catch(e) { toast(e.message); }\n}\nasync function deleteNotice(id) {\n  if (!confirm('공지 삭제할까요?')) return;\n  try {\n    await api('/admin/api/notice-delete', { method:'POST', body: JSON.stringify({ id:id }) });\n    toast('삭제 완료');\n    loadAll();\n  } catch(e) { toast(e.message); }\n}\nfunction renderNotices() {\n  var rows = (DATA.notices || []).map(function(n){\n    return '<tr><td><b>' + esc(n.title) + '</b></td><td>' + esc(n.body).slice(0,80) + '</td><td>' + (n.active !== false ? '<span class=\"pill\">노출</span>' : '<span class=\"pill\">숨김</span>') + '</td><td><button type=\"button\" class=\"btn light js-edit-notice\" data-id=\"' + esc(n.id) + '\">수정</button> <button type=\"button\" class=\"btn danger js-delete-notice\" data-id=\"' + esc(n.id) + '\">삭제</button></td></tr>';\n  }).join('');\n  var body = $('noticeRows');\n  if (!body) return;\n  body.innerHTML = rows || '<tr><td colspan=\"4\" class=\"muted\">등록된 공지가 없습니다.</td></tr>';\n  body.querySelectorAll('.js-edit-notice').forEach(function(btn){ btn.addEventListener('click', function(){ editNotice(btn.getAttribute('data-id')); }); });\n  body.querySelectorAll('.js-delete-notice').forEach(function(btn){ btn.addEventListener('click', function(){ deleteNotice(btn.getAttribute('data-id')); }); });\n}\n\nasync function uploadImage() {\n  const f = $('uploadFile').files[0];\n  if (!f) { toast('이미지를 선택하세요'); return; }\n  const fd = new FormData();\n  fd.append('image', f);\n  try {\n    const r = await fetch('/admin/api/upload', { method:'POST', body: fd });\n    const j = await r.json();\n    if (!r.ok || !j.ok) throw new Error(j.error || '업로드 실패');\n    toast('업로드 완료');\n    $('uploadFile').value = '';\n    await loadAll();\n    if (navigator.clipboard) navigator.clipboard.writeText(j.url).catch(function(){});\n  } catch(e) { toast(e.message); }\n}\nfunction renderImages() {\n  var html = (DATA.images || []).map(function(img){\n    return '<button type=\"button\" class=\"js-copy-image\" data-url=\"' + esc(img.url) + '\"><img src=\"' + esc(img.url) + '\"><span>' + esc(img.name || img.url) + '</span></button>';\n  }).join('');\n  var grid = $('imageGrid');\n  if (!grid) return;\n  grid.innerHTML = html || '<p class=\"muted\">업로드한 이미지가 없습니다.</p>';\n  grid.querySelectorAll('.js-copy-image').forEach(function(btn){ btn.addEventListener('click', function(){ copyImage(btn.getAttribute('data-url')); }); });\n}\nfunction copyImage(url) {\n  if (navigator.clipboard) navigator.clipboard.writeText(url).catch(function(){});\n  toast('이미지 URL 복사: ' + url);\n}\nfunction pickImageFor(inputId) {\n  showTab('images');\n  toast('이미지를 업로드하거나 클릭해서 URL을 복사한 뒤 입력칸에 붙여넣으세요.');\n}\n\nasync function loadFiles() {\n  try {\n    const j = await api('/admin/api/files');\n    FILES = j.files || [];\n    renderFiles();\n  } catch(e) { toast(e.message); }\n}\nfunction renderFiles() {\n  var search = $('fileSearch');\n  var q = (search ? search.value : '').toLowerCase();\n  var html = (FILES || []).filter(function(f){ return f.path.toLowerCase().includes(q); }).map(function(f){\n    return '<button type=\"button\" class=\"file-item js-load-file\" data-path=\"' + esc(f.path) + '\">' + esc(f.path) + '<br><span class=\"muted\">' + Math.round(f.size / 1024) + 'KB</span></button>';\n  }).join('');\n  var list = $('fileList');\n  if (!list) return;\n  list.innerHTML = html || '<p class=\"muted\" style=\"padding:12px\">파일이 없습니다.</p>';\n  list.querySelectorAll('.js-load-file').forEach(function(btn){ btn.addEventListener('click', function(){ loadFile(btn.getAttribute('data-path')); }); });\n}\nasync function loadFile(p) {\n  if (!p) return;\n  try {\n    const j = await api('/admin/api/file?path=' + encodeURIComponent(p));\n    currentFilePath = p;\n    $('currentFile').textContent = p;\n    $('fileContent').value = j.content;\n    toast('파일 불러옴');\n  } catch(e) { toast(e.message); }\n}\nasync function saveFile() {\n  if (!currentFilePath) { toast('파일을 먼저 선택하세요'); return; }\n  if (!confirm(currentFilePath + ' 파일을 저장할까요?')) return;\n  try {\n    await api('/admin/api/file', { method:'POST', body: JSON.stringify({ path: currentFilePath, content: $('fileContent').value }) });\n    toast('파일 저장 완료');\n  } catch(e) { toast(e.message); }\n}\nasync function changePassword() {\n  try {\n    await api('/admin/api/password', { method:'POST', body: JSON.stringify({ oldPassword:$('oldPass').value, newPassword:$('newPass').value }) });\n    $('oldPass').value = '';\n    $('newPass').value = '';\n    toast('비밀번호 변경 완료. 다시 로그인하세요.');\n  } catch(e) { toast(e.message); }\n}\n";
}
