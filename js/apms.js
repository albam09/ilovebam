...
if (typeof(SIDEVIEW_JS) == 'undefined') { // 한번만 실행
    ...
    function SideView(targetObj, curObj, mb_id, name, email, homepage) 
    {
        ...
        if (g5_is_admin == "super") {
            ...
            if (mb_id) {
                ...
                this.insertTail("delmem", "<a href=\"javascript:delete_and_block('"+mb_id+"','"+name+"');\" rel=\"nofollow\">삭제및탈퇴</a>");
				this.insertTail("shop_info", "<a href=\"javascript::;\" onclick=\"win_point('"+g5_admin_url+"/shop_modify.php?mb_id="+mb_id+"')\" >업소정보변경</a>");
				this.insertTail("ban", "<a href=\"javascript::;\" onclick=\"m_del('"+mb_id+"')\" >강퇴</a>");
			}
		}
    }
	
	function delete_and_block_confirm(mb_id) {
		showloading();
		$.ajax({
			url:'/ajax/ajax_user_reject.php',
			type: 'POST',
			data: 'mb_id='+mb_id,
			cache:false,
			dataType: "text",
			...
			success:  function(result){
				...
				if (result=='INVALIDPARAMETER') {
					...
					salert('error','','삭제할 자료가 없습니다');
					return false;
				} else if (result=='ERR') {
					salert('error','','오류가 발생하여 처리하지 못했습니다');
					return false;
				}
				salert('success','','삭제/탈퇴가 완료되었습니다','','document.location.reload();');
				return false;
			},
	...
	function delete_and_block(mb_id,mb_nick) {
		if (typeof mb_id!='undefined') {
			swal.fire({
				title: "",
				text: mb_nick+" ("+mb_id+") 유저의 모든 작성글과 탈퇴작업을 진행하겠습니까?",
				icon: "warning",
				...
				if (result.isConfirmed) {
					delete_and_block_confirm(mb_id);
					return false;
				}
			});
		}
	...
	function m_del(id){
		if (!confirm(id+" 강퇴하시겠습니까?")) {
			return false;
		} else {
			location.href=g5_admin_url+"/member_ban.php?id="+id;
		}
	}
...
function apms_good(bo_table, wr_id, act, id, wc_id) {
	...
	$.post(href, { js: "on" }, function(data) {
		if(data.error) {
			salert('info','',data.error);
			return false;
		} else if(data.success) {
			salert('success','',data.success);
